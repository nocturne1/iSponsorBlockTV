...
def playback_state(
    *, video_id="video", state=1, current_time=0.0, duration=10.0
):
...
class EndPauseTests(unittest.IsolatedAsyncioTestCase):
    async def test_playing_video_schedules_pause_before_end(self):
        ...
        with patch(
            "iSponsorBlockTV.main.time.monotonic", side_effect=[100.0, 100.0]
        ):
        ...
        video_id, delay, expected_end = (
            listener._pause_current_video_before_end.await_args.args
        )
    ...
    async def test_long_video_refreshes_position_before_pausing(self):
        ...
        self.assertEqual(sleep.await_args_list, [call(40.0), call(58.5)])
        listener.lounge_controller.pause.assert_awaited_once_with()
